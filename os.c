...
// **similar to Lab 3. initialize priority field****
  ...
  status = StartCritical();
		...
		tcbs[3].next = &tcbs[4]; // 2 points to 0
		tcbs[4].next = &tcbs[5]; // 2 points to 0
		tcbs[5].next = &tcbs[6]; // 2 points to 0
		tcbs[6].next = &tcbs[7]; // 2 points to 0
		tcbs[7].next = &tcbs[0]; // 2 points to 0
...
// If there are multiple highest priority (not blocked, not sleeping) run these round robin
	...
	for(i=NUMTHREADS;i>0;i--){
		 TmpPtr = TmpPtr->next;    
			...
			if( (TmpPtr->Priority < max) && (TmpPtr->Blocked == 0) && (TmpPtr->Sleep == 0)){
				RunPt =TmpPtr; // this is the thread to run
				max = TmpPtr->Priority;
			}
	}*/
...
void OS_EdgeTrigger_Init(int32_t *semaPt, uint8_t priority){
	...
	edgeSemaphorePri = priority;
	
//***IMPLEMENT THIS***
	//p1.1 == 1 on 432
	//p1.4 == 2
	
	*edgeSemaphore = 0;     // (a) initialize semaphores
...
void PORT1_IRQHandler(void){
	//J4 32 2										P3.5  !!! SHOULD HAVE READ THESE REALLY!!!
	//j4 33 1 BTN n EDU BOOSTER P5.1
	
	int status;
	  status = P1IV;     // 4 for P1.1 and 10 for P1.4
  if(status == 4){
    OS_Signal(edgeSemaphore); // SW1 occurred
		P1IE &= ~0x02;    // disarm interrupt on P1.1
  }
  if(status == 10){
    OS_Signal(edgeSemaphore); // SW2 occurred
		P1IE &= ~0x10;    // disarm interrupt on P1.4
  }
}

// Balls up I used wrong btns this returns btn state
int OS_Btn(int n)
{
	if(n==1){ // BTN 1
		return P1IN&0x02;
